...
#
# Student: Shen Lihang, Student Number:1004911438
#
...
.data
	...
	#flea
	fleaCount: .word 0	 # | 当为20的时候，随机生成一个flea，并再次让fleaCount变成0 |
	flea_current_location: .word 0
	flea_old_location: .word -1
.text 
...
random_mushrooms: # |随机生成20个蘑菇|
 ...
 draw_mushrooms:
  ...
  get_random_flea: # |在11 到 928 之间 （inclusive）|
	addi $sp, $sp, -4
	sw $ra, 0($sp)
	
	li $v0, 42         # Service 42, random int bounded
  	li $a0, 0          # Select random generator 0
  	li $a1, 32       # |生成从0到31的随机数|     
  	syscall             # Generate random int (returns in $a0)
 ...
 flea_delay: # |暂时不知道干啥的|
	...
	li $a1, 2000 # change a2 to a1
flea_delay2:
	addi $a1, $a1, -1 # change a2 to a1
	bgtz $a1, flea_delay2 # change a2 to a1
  ...
  creat_flea:
	...
	sw $ra, 0($sp)
	#==============================================================
	jal get_random_flea  # | $a0 存储了随机数 |

	add $t1, $zero, $a0	# | $t1 代表了flea current的位置 |
	
	# | 存储 $t1 到 flea_current_location |
	la $t3,flea_current_location
	sw $t1,0($t3)
	
	# | 创建一个loop（运行31次），来画出flea的轨迹 |
	# | 每次画当前位置的时候，判断当前位置是否有 蘑菇 或者 蜈蚣，如果有的画，跳过画当前这格，并进入下次loop |
	# | 在最后一次，如果location等于player location， 则游戏结束 |
	
	addi $t2, $zero, 0 # | $t2 是 i |
	addi $t5,$zero,32 # | exit condition 是当 i 为 31的时候 |
	flea_loop:
		jal flea_delay
		
		la $t3 flea_old_location
		lw $t4, 0($t3)
		
		# | 当 old flea location 不等于-1 时，我们把old location 画成黑色 |
		beq $t4,-1,flea_next
	...
	flea_next:
		# | 得到每个list里第i个element的index |
		beq $t2,$t5,flea_exit  # | 如果i等于31，则exit这个for loop |
		sll $s0, $t2, 2     # | 现在 s0 代表了每个list里面的第 i 个 element 的 index |
		lw $s1, displayAddress
		
		# | 用loop判断当前位置有没有 蘑菇 |
		check_present_touch_mushroom:
			# | 关于蘑菇的信息 |
			la $t3, mushroomsLocation
			la $t4, mushroomsExist
			...
			addi $s5, $zero,20	# | loop exit cond is 20 |
		flea_mushroom_loop:
			beq $s4,$s5, flea_before_update	# | for loop head |
			sll $s6,$s4,2
			...
			beq $t1,$s7,flea_check_mushroomExit # | 如果flea的现在的位置是蘑菇，跳转到flea_check_mushroomExit |
			
			j flea_mushroom_update # | 如果第 i 个蘑菇和现在位置不冲突， 找下一个蘑菇 |
			
		flea_check_mushroomExit:
			la $t4, mushroomsExist
			sll $s6, $s4, 2
			add $s6,$s6,$t4
			lw $s7, 0($s6)
			...
			beq $s7,1,flea_update# | 存在蘑菇，所以不画当前位置 |
			
			# | 运行到这表示当前位置没有蘑菇， 所以涂成flea的颜色 |
			li $t8, 0xa05800
			sll $t9,$t1,2
			add $t9, $t9, $s1
			sw $t8, 0($t9)
			# | 把当前位置存到old位置上 |
			la $t7, flea_old_location
			sw $t1,0($t7)
			
			j flea_update
			# | 在检测到了当前位置不存再蘑菇后，现在把当前位置画成flea |
		flea_mushroom_update:
			addi $s4,$s4,1
			j flea_mushroom_loop
