...
def lemon(xv):
    ...
    def helper(xv):
        if isinstance(xv, int):
            return xv
        # notice that, this for loop only work for the self pointer part
        # such as x=[1,2]; x.append(x); lemon(x)
        for old_new in lemon_lookup:
            # for example, if xv is [1,2,[...]], then when go though the [...] part
            # this for loop will return new_xv which will allow last layer, the "new_xv.append()"" part, point to itself 
            # and it perfectly solve creating a clone of the self-pointing newsted list problem
            if old_new[0] is xv:
                return old_new[1]
        new_xv = []
    ...
    #     lemon_lookup.append((xv, new_xv))
    #     for element in xv:
    #         new_xv.append(helper(element)) if isinstance(element,list) else new_xv.append(element)
    #     return new_xv
    # return helper(xv)
