...
class Place:
    ...
    We create tunnel from left to right (0-->n)
    """
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        selected_place = self.place
        cnt = 0
        while selected_place != beehive:
            if selected_place.bees and self.min_range <= cnt <= self.max_range:
                return rANTdom_else_none(selected_place.bees)

            selected_place,cnt = selected_place.entrance,cnt+1
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= amount:
            death_damage = self.damage
        else:
            death_damage = 0
            
        for bee in self.place.bees[:]:
            bee.reduce_armor(amount+death_damage)
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        if not isinstance(other,ContainerAnt) and not self.contained_ant:
            return True
        # END Problem 9
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if not QueenAnt.after_first:
            self.true_queen = True
            QueenAnt.after_first = True
        else:
    ...
    def action(self, gamestate):
        ...
        # increase damage once every two turn, notice for the container ant
            current_place = self.place.exit
            while current_place is not None:
                if current_place.ant and not current_place.ant.buffed:
                    current_place.ant.damage *= 2
                    current_place.ant.buffed = True
                if isinstance(current_place.ant,ContainerAnt) and current_place.ant.contained_ant and not current_place.ant.contained_ant.buffed:
                    current_place.ant.contained_ant.damage *= 2
                    current_place.ant.contained_ant.buffed = True
                current_place = current_place.exit
        # if it is the impostor, it will die
        else:
    ...
    def remove_from(self, place):
        if not self.armor > 0 or not self.true_queen:
